...
const analyzer = new TaskAnalyzer(new CurrentAssigneeResolver(), new WorkflowStatusResolver(), new LeadTimeResolver(), new InactiveDaysResolver());
...
for (const snapshot of snapshots) {
    statusCount.set(snapshot.status, (statusCount.get(snapshot.status) ?? 0) + 1);
}
console.log("========================================");
...
console.log("-----------------------------");
const pending = snapshots.filter(snapshot => snapshot.status !== TaskStatus.PUBLISHED);
